...
$sonuc = $sorgu->fetch();//sorgu çalıştırılıp veriler alınıyor
        ...
        if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
            ...
            if ($baslik <> "" && $aciklama <> "" && $hata == "") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
              ...
              $sql = "UPDATE servisler SET resim=:resim, baslik=:baslik, aciklama=:aciklama WHERE id=:id;";     
              $durum = $baglanti->prepare($sql)->execute($satir);

              if ($durum)
              {
               echo '<script>swal("Başarılı","Güncellendi","success").then((value)=>{ window.location.href = "servisler.php"});
